...
public class GUICor extends JDialog {
    ...
    private CorTableModel tableModel;

    private DAOCor daoCor = new DAOCor();
    ...
    public GUICor() {
        ...
        tableModel = new CorTableModel(lista);
...
//        painelBotoes.add(btnRem);
            ...
            public void actionPerformed(ActionEvent evt) {
                if (table.getSelectedRow() >= 0) {
                    ...
                    if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(cp,
                            "Confirma a exclusão desse tipo de conta [" + tableModel.getValue(table.getSelectedRow()).getIdCor() + " - "
        ...
        btnAdd.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    Cor c = daoCor.obter(cor.getIdCor());
                    if (c == null) {
                        daoCor.inserir(cor);
                        tableModel.onAdd(cor);
                        tableModel.fireTableDataChanged();
                    } else {
                        JOptionPane.showMessageDialog(cp, "Já existe esse tipo conta");
                    }
                } catch (Exception err) {
                    JOptionPane.showMessageDialog(cp, "Erro ao inserir a conta =>" + cor.getIdCor()
        ...
        btnRem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (table.getSelectedRow() != -1 && table.getSelectedRow() < tableModel.getRowCount()) {
                    Cor c = tableModel.getValue(table.getSelectedRow());
                    daoCor.remover(c);
